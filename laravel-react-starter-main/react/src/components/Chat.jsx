...
const ChatApp = () => {
  ...
  const formatMessageForLibrary = (msg) => {
    ...
    // If message is already formatted (has type='photo' or type='file' with proper structure), return as-is
    ...
    // Ensure date is always a Date object for react-chat-elements
    let date = msg.date;
    if (!date || !(date instanceof Date)) {
      date = parseTimestamp(msg.sent_at || msg.created_at || msg.date);
    }
    // Format date string for display (HH:mm:ss format)
    const hours = String(date.getHours()).padStart(2, '0');
    const minutes = String(date.getMinutes()).padStart(2, '0');
    const seconds = String(date.getSeconds()).padStart(2, '0');
    const dateString = `${hours}:${minutes}:${seconds}`;
    // Use sender_name from message if available (from WebSocket notifications), 
    // otherwise fall back to recipient name or 'User'
    const title = isCurrentUser 
      ? (user?.name || 'You') 
      : (msg.sender_name || recipient?.name || 'User');

    // Handle file/image messages - use 'photo' type for images, 'file' for other files
    if (msg.type === 'file' || msg.is_file === true || msg.is_file === 1) {
      ...
      const isImage = fileType.startsWith('image/') || 
        ...
        // Debug: Log image detection details (can be removed later)
        ...
        //   fileTypeStartsWithImage: fileType.startsWith('image/'),
        //   extensionMatch: /\.(jpg|jpeg|png|gif|webp)$/i.test(fileName)
        // });

      if (isImage) {
        // Use 'photo' type for images (react-chat-elements PhotoMessage component)
        // PhotoMessage expects: type='photo', data.uri, and optionally text (shown below image)
        // Don't set fixed width/height - let CSS handle responsive sizing
        const photoMessage = {
          position,
          type: 'photo',
          title,
          ...
          data: {
            ...
            // PhotoMessage CSS will handle max-height: 300px
            alt: fileName || 'Image'
          },
          user_id: msg.user_id
        };
        
        // Only add text if it exists (text shows below the image)
        if (msg.text && msg.text.trim()) {
          photoMessage.text = msg.text;
        }
        
        // Debug logs (can be removed later)
        // console.log('Photo message created:', photoMessage);
        // console.log('Photo URL:', fileUrl);
        return photoMessage;
      } else {
        // Use 'file' type for non-image files
        return {
          position,
          type: 'file',
          title,
  ...
  const scrollToBottom = () => {
    // Use multiple approaches to find and scroll the message list
    if (messageListRef.current) {
      // Try to access the scrollable element directly
      const element = messageListRef.current;
      
      // Method 1: Try direct scroll
      if (element.scrollTop !== undefined) {
        element.scrollTop = element.scrollHeight;
      }
      
      // Method 2: Try to find scrollable children
      ...
      // Method 3: Try react-chat-elements specific classes
      const rceElement = element.querySelector('.rce-mlist') || 
                        element.querySelector('.rce-container-mlist') ||
                        element.querySelector('.rce-container-body');
      if (rceElement && rceElement.scrollTop !== undefined) {
        rceElement.scrollTop = rceElement.scrollHeight;
      }
    }
    
    // Method 4: Try to find by className
    const messageListElements = document.querySelectorAll('.message-list, .rce-mlist, .rce-container-mlist');
    messageListElements.forEach(el => {
  ...
  const loadChatHistory = async () => {
    if (!user || !user.id) {
      console.log('User not available yet, skipping chat history load');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const formattedMessages = response.data.map(msg => {
        const isCurrentUser = parseInt(msg.user_id) === parseInt(user.id);
        console.log(`Message from user ${msg.user_id} (${typeof msg.user_id}), current user ${user.id} (${typeof user.id}), isCurrentUser: ${isCurrentUser}`);
        const date = parseTimestamp(msg.sent_at || msg.created_at);
        const hours = String(date.getHours()).padStart(2, '0');
        const minutes = String(date.getMinutes()).padStart(2, '0');
        const seconds = String(date.getSeconds()).padStart(2, '0');
        const timeString = `${hours}:${minutes}:${seconds}`;
        console.log('msg.is_file:', msg.is_file);
  ...
  const setupEchoConnection = () => {
    ...
    newEcho.private(`App.Models.User.${user.id}`)
      .notification((notification) => {
        ...
        // Process all chat messages for this specific chat
        ...
        if (notification.type === 'App\\Notifications\\NewChatMessage' && parseInt(notification.message.chat_id) === parseInt(chatId)) {
          ...
          console.log('Chat ID:', chatId, 'Message Chat ID:', notification.message.chat_id);
          console.log('Message from user:', notification.message.user_id);
    ...
    return (
      <div style={{
        ...
        <Hearts
          ...
          visible={true}
        />
        <p style={{ color: '#8b4513', fontSize: '18px', fontWeight: 'bold' }}>
          Се вчитува разговорот...
        </p>
      </div>
    );
  }
      ...
      <div className="chat-container">
        ...
        <MessageList
          ...
          dataSource={messages
            ...
            .filter(msg => msg !== null) // Filter out any invalid messages
          }
          formatDate={(date) => {
            // Format date to show only time (HH:mm:ss)
            if (!date) return '';
            try {
        ...
        <div className="input-container" style={{
          ...
          <Button 
            ...
            color='white' 
            backgroundColor={(inputValue.trim() || attachedFile) ? '#007bff' : '#6c757d'}
            text='Испрати' 
            onClick={sendMessage}
